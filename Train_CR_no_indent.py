...
# Readability rule-based labeling function (indentation removed)
def analyze_readability(code: str) -> str:
    ...
    if avg_length < 100 and comment_ratio > 0.05 and nesting_score < 1.5:
        return "readable"
    else:
        return "unreadable"
...
for folder in tqdm(os.listdir(DATA_PATH), desc="📁 Loading Projects"):
    ...
    if os.path.isdir(folder_path):
        for file in tqdm(os.listdir(folder_path), desc=f"📄 {folder}", leave=False):
...
callbacks = [
    ...
    tf.keras.callbacks.ModelCheckpoint('best_coderead_no_indent.h5', monitor='val_accuracy', save_best_only=True)
]
...
print(f"\n✅ Test Accuracy: {acc:.2%}")
model.save("coderead_no_indent.h5")
print("\U0001F4BE Model saved as coderead_no_indent.h5")
